...
def load_data_from_subject_csv(root_dir, output_name):
    ...
    for ctn in root_ctn:
        if ctn.endswith('.csv'):
            csvs.append(os.path.join(root_dir, ctn))
    ...
    for csv_file in csvs:
        with open(csv_file, 'r') as csv_in:
            ...
            next(lines)
            for line in lines:
                if line[3] == "1" and line[2] != '' and line[2] != '--undefined--' and line[1] != '' and (float(line[2]) <= 1500):
...
def load_features(dataset, noise, w1, w2, w3, verbose):
    mfcc_list = []
    for idx, wav in enumerate(tqdm(dataset.location)):
        _, _, _, mfcc = features.get_features(wav, noise, w1, w2, w3)
